...
| `USGov` (GCC) | `https://api.powerbigov.us` | **`https://login.microsoftonline.com`** | `https://analysis.usgovcloudapi.net/powerbi/api` | `powerbi://api.powerbigov.us` | `https://api.fabric.microsoft.us` (unverified - Fabric is not offered in GCC; the run detects that and skips Fabric calls, see Data-Coverage.md) | `AzureCloud` |
