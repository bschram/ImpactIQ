...
| 7 | Settings file `Config\ImpactIQ.Settings.json` (parameter-name keys; parameter > env var > file > default; typed, ValidateSet-checked, blocked keys refused); the environment picked in the dialog is written back and pre-selected next time (`-Default` on `Select-IQEnvironmentInteractive` / `Show-EnvironmentSelectionDialog`); `ImpactIQ.bat` launcher with `--no-prompts`, `--resume`, `--fresh`, `--environment`, `--settings`, `--help` | `Get-IQEntrySettingsPath`, `Read-IQEntrySettings`, `Set-IQEntryDefaultsFromSettings`, `Save-IQEntrySetting` (entry point), Interactive, `ImpactIQ.bat`, `Config\ImpactIQ.Settings.example.json` |
...
| 2026-09-22, that script on the same GCC capacity: the Power BI PowerShell module token (client `23d8f6bd-…`, Power BI service scopes) is accepted in both forms; the Az.Accounts token for the same audience is refused. The token source is now a probe dimension: `SignIn` / `PowerBIModule`, module variants first; `-XmlaTokenSource Auto\|SignIn\|PowerBIModule` (`IMPACTIQ_XMLA_TOKEN_SOURCE`, settings `XmlaTokenSource`); the accepted connection is remembered in `State\xmla-connection.json` (per environment) and used from the first export of later runs, falling back to the sign-in token with a Warn when the module cannot sign in headless | Auth (`Get-IQPowerBIModuleXmlaToken`: Provider Module -> regular token; existing module session; stored credential; browser when interactive), Models (`Get-IQModelXmlaConnection`, `Get-IQModelXmlaToken`, `Get-IQModelXmlaVariantList`, `Get-IQModelXmlaMemory` / `Save-IQModelXmlaMemory`), entry point |
