...
Describe 'ImpactIQ.ps1 parses and declares the headless parameters' -Skip:(-not $script:HasEntry) {
    ...
    It 'declares the recovered-build port parameters (exclusions, quarantine, keep-awake)' {
        $params = @($script:Ast.ParamBlock.Parameters | ForEach-Object { $_.Name.VariablePath.UserPath })
        foreach ($p in @('ExcludeWorkspaceId', 'ExcludeWorkspaceName', 'NoQuarantine', 'NoKeepAwake', 'NoWebUiExportFallback', 'WebUiClusterHost', 'SettingsPath', 'XmlaTokenResource', 'XmlaTokenSource', 'NoXmlaProbe')) { $params | Should -Contain $p }
