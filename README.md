...
## ⚙️ Launch Options and Settings

### `ImpactIQ.bat` command-line options

Run from the ImpactIQ base folder:

- `ImpactIQ.bat --help` (or `-h`, `/?`) → shows all launcher options and exits.
- `ImpactIQ.bat --no-prompts` → runs non-interactive using values from `Config\ImpactIQ.Settings.json`.
- `ImpactIQ.bat --overwrite` or `ImpactIQ.bat --append` → choose workbook behavior without prompt.
- `ImpactIQ.bat --resume` or `ImpactIQ.bat --fresh` → choose checkpoint behavior without prompt.

Example unattended run:

`ImpactIQ.bat --no-prompts --append --resume`

### `Config\ImpactIQ.Settings.json`

This file is the primary configuration source. It now includes `_comments` entries that describe valid values/formats.

Key settings:

- `BaseFolderPath` → absolute working/output folder (replaces hardcoded `C:\Power BI Backups`).
- `LastEnvironment` → one of: `Public`, `Germany`, `USGov`, `China`, `USGovHigh`, `USGovMil`.
- `IncludeWorkspaceIds` / `ExcludeWorkspaceIds` → arrays of workspace GUIDs.
- `IncludeMyWorkspace` → `true` / `false`.
- `WorkbookMode` → `append` or `overwrite`.
- `ResumeMode` → `resume` or `fresh`.
- `Notifications.Enabled` → enables/disables re-auth notifications.
- `Notifications.Method` → `SMTP` or `OUTLOOK`.
- `Notifications.EmailRecipients` / `Notifications.SmsRecipients` → arrays of recipient addresses.

> Notification settings are now configured through `ImpactIQ.Settings.json` instead of `ImpactIQ.Notify.env`.
