...
|---|---|---|
| `-BaseFolder <path>` | `IMPACTIQ_BASE_FOLDER`, else the folder `ImpactIQ.ps1` runs from | root for `Config\`, `State\`, `Logs\` and, by default, `Outputs\` with the backup folders and the four workbooks. `C:\Power BI Backups` is no longer assumed (it is only tried when the script text was pasted into a console and that folder still holds `ImpactIQ.ps1`) |
| `-BackupFolder <path>` | `IMPACTIQ_BACKUP_FOLDER`, else `<BaseFolder>\Outputs` | where `Model Backups\`, `Report Backups\` and `Dataflow Backups\` go; a relative path is created under the base folder (`-BackupFolder Data` -> `<BaseFolder>\Data\Model Backups\...`). The csx scripts receive it as `IMPACTIQ_BASE` |
| `-OutputFolder <path>` | `IMPACTIQ_OUTPUT_FOLDER`, else `<BaseFolder>\Outputs` | where the four workbooks go; point the `.pbit`'s `Base Directory` there |
...
| `-NoWebUiExportFallback` | off | do not retry a report the Export API refused with `PremiumFiles` (large storage format) through the portal's own export endpoint `https://<cluster>/export/v202402/reports/<id>/pbix` (undocumented; polled for a download URL, then downloaded; method `WebUI`) |
| `-WebUiClusterHost <host>` | from the report URL redirect | pins the `wabi-...analysis...` cluster host used by that fallback |
| `-XmlaTokenResource <url>` | `IMPACTIQ_XMLA_RESOURCE`, else the environment's Power BI resource | audience of the token Tabular Editor uses for XMLA exports (e.g. `https://analysis.windows.net/powerbi/api`); pins it, so only the other connection-string form is probed after an authentication failure |
| `-XmlaTokenSource Auto\|SignIn\|PowerBIModule` | `IMPACTIQ_XMLA_TOKEN_SOURCE`, else `Auto` | source of the token Tabular Editor uses for XMLA exports: the run's sign-in, or MicrosoftPowerBIMgmt (`Connect-PowerBIServiceAccount` / `Get-PowerBIAccessToken`), which the GCC XMLA endpoint accepts where the Az token is refused. `Auto` tries the module token first after a refusal and starts later runs with the connection remembered in `State\xmla-connection.json`. The module needs a sign-in: headless runs get it with `-AuthMode Credential` only; otherwise the remembered connection falls back to the sign-in token with a Warn line |
| `-NoXmlaProbe` | off | after "Authentication failed for all authenticators", do not try the module token / other audience / `Password=`-only form once per run |
| `-SettingsPath <file>` | `IMPACTIQ_SETTINGS_PATH`, else `<BaseFolder>\Config\ImpactIQ.Settings.json` | JSON whose keys are parameter names; a value is the default for a parameter not given on the command line or by its environment variable (parameter > variable > file > built-in). `Environment` in an interactive run only pre-selects the dialog and the pick is written back. `BaseFolder`, `SettingsPath`, `Credential`, `TokenCacheKey`, `Force`, `RunId` are never read from it |
...
<BaseFolder>\
  ...
  Outputs\Model Backups\<RunId>\<Ws> ~ <Model>.bim, .csv, _MD.csv          } -BackupFolder moves these
  Outputs\Report Backups\<RunId>\<Ws> ~ <Report>.pbix|.rdl, *.txt (report detail extracts), ReportExports.txt
  Outputs\Dataflow Backups\<RunId>\<Ws> ~ <Dataflow>.txt|.pq, <name>.definition\, Dataflow Detail.xlsx
  Outputs\Power BI Environment Detail.xlsx, Report Detail.xlsx, Model Detail.xlsx, Dataflow Detail.xlsx   } -OutputFolder moves these
```

`Config\`, `State\` and `Logs\` always stay in the base folder; everything a run produces goes under `Outputs\`, and
only the three backup folders (`-BackupFolder`) and the four workbooks (`-OutputFolder`) can be moved. Checkpoints record absolute output paths, so keep the same folders
